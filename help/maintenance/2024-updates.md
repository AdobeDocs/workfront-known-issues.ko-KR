---
title: 2024년 Workfront 유지 보수 업데이트
...
세부 정보 페이지와 매트릭스 페이지 모두에서 프로젝트(재무 데이터) 보고서에 대해 죄송합니다 오류가 간헐적으로 나타납니다.

이 문제를 해결하는 과정에서 이 보고서 유형의 프로젝트는 더 이상 데이터를 로드하기 전에 자동으로 다시 계산되지 않습니다. 사용자가 재무 데이터 보고서의 데이터를 업데이트하려면 개별 프로젝트의 재무를 수동으로 다시 계산해야 합니다.
