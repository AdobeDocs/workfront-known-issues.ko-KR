...
+++**(예정) 2024년 2월 1일 금요일 유지 보수 업데이트**

### (예정) 2024년 2월 1일 유지 보수 업데이트

#### 로그인

**SSO를 사용하는 사용자는 로그인할 때 원래 위치로 리디렉션되지 않습니다**

사용자가 다음 위치의 페이지에 있을 때 [!DNL Workfront] 및 SSO로 로그인하면 로그인이 완료됩니다. [!UICONTROL 홈] 로그인하기 전에 있었던 페이지 대신
...
+++**2024년 1월 30일 수요일 유지 보수 업데이트 (핫픽스)**

### 2024년 1월 30일 유지 보수 업데이트 (핫픽스)

#### 보고서

**외부 API 필드에 목록 및 보고서에 사용 가능한 모든 값이 표시되지 않음**

이전에는 사용자가 목록 및 보고서에서 외부 조회 필드에 대해 선택한 값을 보고 값을 편집할 수 있었지만 API에서 제공되는 옵션이 있는 드롭다운은 표시되지 않았습니다.

이제 외부 조회 사용자 정의 필드를 목록 또는 보고서에 사용하면 외부 API의 모든 옵션이 있는 드롭다운을 사용할 수 있습니다.
