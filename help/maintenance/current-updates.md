...
**종속 작업에서 전임 작업을 이동할 때 오류 메시지가 표시되지 않음**

_작업_

사용자가 작업 목록에서 작업 순서를 변경하려고 하는데 해당 작업이 [!UICONTROL 승인 대기 중] 상태인 경우, 작업이 작업 목록에서 이동하는 것으로 나타납니다. 새로 고침 시 사용자는 항목이 이동하지 않았음을 확인할 수 있습니다. 전임 작업은 전임 작업인 작업 아래로 이동할 수 없으므로 항목을 이동할 수 없지만 사용자에게 항목을 이동할 수 없다는 메시지가 표시되지 않아서 혼동이 발생할 수 있습니다.
