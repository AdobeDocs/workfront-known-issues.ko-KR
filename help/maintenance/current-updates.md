...
문제를 작업으로 변환할 때 자신을 참조하는 계산된 필드는 해당 값을 유지하지 않습니다.

문제를 작업으로 변환할 때 편집 창에 원하는 값이 올바르게 표시됩니다. 그러나 변환이 완료되면 계산된 필드에 &quot;N/a&quot;가 표시됩니다.

**에서 필터를 변경할 때 오류 발생 [!UICONTROL 홈]**

_홈_

사용자가 다음에 대한 필터를 변경할 때 [!UICONTROL 홈], [!UICONTROL 홈] 영역이 로드되지 않고 다음과 같은 오류가 표시됩니다.
...
보관된 카드는 표시되지 않더라도 보드 로드 시간에 영향을 주었습니다.
...
**카드를 접수 열에서 이동할 수 없습니다.**

_보드_

사용자는 접수 열에서 한 카드를 보드의 다른 열로 드래그할 수 있지만, 후속 카드는 접수 열에서 이동할 수 없습니다.

**보드 성능에 영향을 주는 그룹화 기준**

_보드_

사용자가 할당자 또는 태그로 카드를 그룹화하려고 하면 보드 성능이 매우 느려집니다.
...
사용자는 최신 버전을 수동으로 다운로드할 수도 있습니다. 자세한 내용은 [데스크탑 증명 뷰어 설치](https://experienceleague.adobe.com/docs/workfront/using/review-and-approve-work/proofing/use-the-desktop-proofing-viewer/installing-desktop-proofing-viewer.html?lang=en)를 참조하십시오.
