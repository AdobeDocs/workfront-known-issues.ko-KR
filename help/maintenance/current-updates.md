...
+++**2024년 12월 5일 금요일 유지 보수 업데이트**

### 2024년 12월 5일 유지 보수 업데이트
...
**표시 논리가 사용자 정의 양식에서 작동하지 않음**

사용자가 표시 논리를 사용하여 사용자 정의 양식에 정보를 입력하면 표시 논리를 기반으로 표시되어야 하는 필드가 표시되지 않습니다.

#### 홈

**기여자가 작동하지 않는 &quot;처리 중&quot; 단추를 표시함**

참여자 사용자는 홈의 내 작업 위젯에서 작동하지 않는 &quot;작업 수행&quot; 버튼에 액세스할 수 있습니다.

#### 보고서

**사용자 정의 분기가 보고서 프롬프트에 표시되지 않음**

사용자가 날짜 기반 프롬프트로 보고서를 실행할 때 사용자 정의된 분기가 프롬프트에 대한 선택 목록에 표시되지 않습니다.
...
**범주 이름 필드가 사용자 정의 양식에 연결되지 않음**

카테고리 이름 필드가 포함된 보고서에서 목록을 볼 때 카테고리 이름 열의 콘텐츠를 클릭하여 해당 사용자 정의 양식을 열 수 없습니다.

**보고서에서 사용자 지정 색이 작동하지 않음**

Report Builder에서 그룹에 사용자 정의 색상을 할당할 때 보고서를 실행할 때 색상 선택 사항이 표시되지 않습니다.

**보고서에 대한 긴 로드 시간**

보고서를 로드하는 데 시간이 오래 걸립니다. 이는 Report Builder에서 보고서를 확인하고, 편집하고 작성하는 데 영향을 미칩니다.

**보고서 프롬프트의 계산된 날짜/시간 필드에 대한 시간 필드가 누락됨**

보고서를 실행할 때 사용자 지정 날짜/시간 필드가 프롬프트로 사용되면 프롬프트 인터페이스에서 시간 선택 필드가 누락됩니다.

**공유된 보고서/대시보드를 볼 수 없음**

사용자가 공유된 보고서 또는 대시보드에 액세스하려고 하면 보고서 또는 대시보드에 액세스할 수 없습니다.

#### 타임시트

**기여자 사용자에게 &quot;승인을 위한 제출&quot; 버튼이 있음**
